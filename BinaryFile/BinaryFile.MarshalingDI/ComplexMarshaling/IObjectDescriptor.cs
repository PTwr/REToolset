...
namespace BinaryFile.MarshalingDI.ObjectMarshaling
{
    public interface IMarshalerStore
    {
        ...
        IWriteMarshaler<TMarshaledType> GetWriteMarshaler<TMarshaledType>();
    ...
    public class DefaultMarshalerStore : IMarshalerStore
    {
        ...
        {
            if (container.TryResolve<IReadMarshaler<TMarshaledType>>(out var exactMarshaler))
            {
                return exactMarshaler;
            }
            ...
            foreach (var type in typeof(TMarshaledType).EnumerateTypeHierarchy().Skip(1))
            {
                ...
                if (container.TryResolve(marshalerType, out var m) && (m is IReadMarshaler<TMarshaledType> marshaler))
                {
                    return marshaler;
                }
            }
        ...
        public IWriteMarshaler<TMarshaledType> GetWriteMarshaler<TMarshaledType>()
        {
            if (container.TryResolve<IWriteMarshaler<TMarshaledType>>(out var exactMarshaler))
            {
                return exactMarshaler;
            }
            ...
            foreach (var type in typeof(TMarshaledType).EnumerateTypeHierarchy().Skip(1))
            {
                ...
                if (container.TryResolve(marshalerType, out var m) && (m is IWriteMarshaler<TMarshaledType> marshaler))
                {
                    return marshaler;
                }
            }
